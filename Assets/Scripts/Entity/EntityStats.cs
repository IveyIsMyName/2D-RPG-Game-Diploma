...
public class EntityStats : MonoBehaviour
{
    ...
    public StatRecourceGroup resources;
    ...
    public float GetPhysicalDamage(out bool isCrit, float scaleFactor = 1)
    {
        ...
        float bonusCritChance = major.agility.GetValue() * .3f; //bonus crit chance from Agility: 0.3% per point
        float critChance = baseCritChance + bonusCritChance;

        float baseCritPower = offense.critPower.GetValue();
        float bonusCritPower = major.strength.GetValue() * .5f; //bonus crit chance from Strength: 0.5% per point
    ...
    public float GetElementalDamage(out ElementType element, float scaleFactor = 1)
    {
        ...
        float bonusElementalDamage = major.intelligence.GetValue(); //Bonus elemental damage from Intelligence +1 per point
    ...
    public float GetElementalResistance(ElementType element)
    {
        ...
        float bonusResistance = major.intelligence.GetValue() * .5f; //bonus resistance from intelligence: +0.5% per point
        ...
        float resistCap = 75f;
        float finalResistance = Mathf.Clamp(resistance, 0, resistCap) / 100;
    ...
    {
        float finalReduction = offense.armorReduction.GetValue() / 100; //Total armor reduction as multiplier
    ...
    public float GetArmorMitigation(float armorReduction)
    {
        ...
        float bonusArmor = major.vitality.GetValue(); //bonus armor from Vitality: +1 per point
        float totalArmor = baseArmor + bonusArmor;

        float reductionMultiplier = Mathf.Clamp(1 - armorReduction, 0, 1); // e.g. 1 - .4f = .6f
    ...
    public float GetEvasion()
    {
        ...
        float bonusEvasion = major.agility.GetValue() * .5f; //bonus evasion from Agility: 0.5% per point
